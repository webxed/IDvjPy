...
SEED_COMMANDS = {
    ...
    "kube": [
        ("tsh kube login CLUSTER", "Авторизация Teleport (NS/POD по кластеру помнит :kctx)"),
        ...
        ("kubectl describe pod $POD -n $NS", "Детали по pod $POD"),
        ("kubectl logs $POD -n $NS", "Логи pod $POD"),
        ("kubectl logs -f $POD -n $NS", "Логи pod $POD (follow, лучше: > cmd)"),
        ("kubectl exec -it $POD -n $NS -- sh", "Exec внутрь pod $POD (лучше: > cmd)"),
        ("kubectl get deploy -n $NS", "Deployments в namespace $NS"),
        ("kubectl describe deploy $DEPLOY -n $NS", "Детали по deployment $DEPLOY"),
        ("kubectl rollout status deploy/$DEPLOY -n $NS", "Статус rollout $DEPLOY"),
        ("kubectl rollout restart deploy/$DEPLOY -n $NS", "Рестарт rollout $DEPLOY"),
        ("kubectl get svc -n $NS", "Services в namespace $NS"),
        ("kubectl port-forward svc/$SVC 8080:80 -n $NS", "Port-forward на service $SVC"),

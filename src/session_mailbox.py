...
старта (первый опрос читает и обнуляет файл). Права файла ``0600``.

Секреты ``$$`` по ящику **не едут**: в команде остаётся имя ``$NAME``, а значение
отправитель кладёт прямо в секретное хранилище цели
(``secrets_<target>.json``, 0600, чистится при выходе её сессии — `app`:
``_pass_secrets_to``). Свои секреты цели не перезаписываются: если имя там уже
есть, команда выполнится с ним.
"""

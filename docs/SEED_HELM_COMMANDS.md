...
`$RELEASE` / `$CHART` / `$VALUES` входят в список переменных кластерного журнала
(`kctx_vars`, по умолчанию вместе с kubectl-стеком `NS POD DEPLOY SVC ING APP CTR QUOTA`).
Значит, после входа в кластер (`klogin …` / `:kctx <cluster>`) присваивание любой из них
пишет снимок в `kctx.json`, а `:kctx <cluster>` / `:kctx N` возвращает релиз вместе
с `$NS` — не надо вспоминать, какой релиз и какой values были в этом кластере.
В снимок попадают только непустые значения, поэтому `$RELEASE=` (пусто) ничего не добавит.
`VALUES` — обычно относительный путь: `:kctx N` вернёт его как есть, так что
запускайте helm-команды из каталога с этим файлом (или задайте абсолютный путь).

...
**IDvjPy_term** v1.178 — 从标签生成命令行的智能终端。
...
term_open: window            # 终端在哪里打开（`:term`、`:new`、`& cmd`）：window —— 新窗口，tab —— 已打开窗口中的标签页（gnome-terminal --tab、konsole --new-tab 等）；$IDVJPY_TERM_OPEN=tab；:term --tab|--window 仅对本会话

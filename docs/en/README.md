...
**IDvjPy_term** v1.176 — a smart terminal for building command lines from tags.
...
| `& cmd` | Run in a **new terminal window** (the TUI keeps running; the output lives in that window, the journal only records the launch) | `& kubectl logs -f pod/api-1`, `& htop` |

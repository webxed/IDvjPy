...
**IDvjPy_term** v1.143 — a smart terminal for building command lines from tags.
...
| `:` | Application commands | `:q`, `:cd`, `:fm`, `:term`, `:session`, `:new`, `:send`, `:welcome`, `:backup`, `:screensaver`, `:r`, `:cmd`, `:kctx`, `:run`, `:playbook`, `:md`, `:rg`, `:lang`, `:relang`, `:?` |
...
- **Output.** In the journal the value is `****`: the block header, the displayed stdout/stderr, `:o` and the `TTY: …` line. At the same time `raw_stdout` is real — `|`, `$OUT` and `F3` work with the real data.
...
Limitations: the whole input line is masked (the name too — it is visible in the subtitle); in an interactive `> cmd` the real terminal shows the value while the TUI is paused; if the command itself prints the secret, the journal shows `****`, but `F3` will give the real output; a line starting with `$$` is always treated as a secret.

...
**IDvjPy_term** v1.178 — a smart terminal for building command lines from tags.
...
- `> cmd` — a real TTY (htop, vim, ssh); a click and PgUp/PgDn activate the visible journal block. A background command does not get the terminal: `stdin` is `/dev/null`, so `read`, `tsh`, `kubectl`, `ssh` do not "steal" keys and the mouse (previously `:kctx` with a hung `tsh kube login` made the input unusable), and on timeout/`F4` the app returns the terminal to its own mode. Need interactivity — `> cmd`, a long task without a TTY — `@ cmd`; `:kctx <cluster>` starts login without `command_timeout`. **That is also why a progress bar is missing from the journal:** without a TTY the tool turns progress off itself — `git clone <url>` prints only `Cloning into …` (plus the server's `remote:` lines, which travel over the data channel), exactly like `git clone … | cat` in a plain terminal. Force it with `git clone --progress …`: every stage (Enumerating/Counting/Receiving/…) stays a separate line. A live bar will not appear anyway: the block is updated when the command finishes (`[Executing...]` → output), and `\r` redraws of one line collapse to the final state (intermediate percentages — hundreds of frames — are dropped on purpose). To watch progress live, use a real TTY: `> git clone …` (TUI paused; that output never lands in the journal).
...
| `& cmd` | Run in a **separate terminal** (window or tab, like `:term`; the TUI keeps running; the output lives there, the journal only records the launch) | `& kubectl logs -f pod/api-1`, `& htop` |
...
- `:h import [shell]` — append the user's shell history to `history_<instance>.txt`: `~/.bash_history`, `~/.zsh_history`, fish (`~/.local/share/fish/fish_history`), ksh (`~/.sh_history`; `sh` is the same file), nushell (system data dir: `Application Support` / `%APPDATA%`), PowerShell PSReadLine (`%APPDATA%\Microsoft\…` on Windows, the XDG path on Linux/macOS), plus the **atuin** database (`history.db`: atuin's data dir — `~/.local/share/atuin` on every OS, overridden by `$ATUIN_DB_PATH` or `db_path`/`data_dir` in `config.toml`; soft-deleted rows are skipped) are searched; a set `$HISTFILE` comes first (format detected from the content even if the file name is unusual). The last 5000 lines of each file (for big files — only the tail) or 5000 atuin commands are taken, lines that are already there are not duplicated (a repeated import adds nothing), nothing is executed. With no name — every source found, `:h import zsh` — that one only, `:h import atuin` — the atuin database only. Failures are explicit: a locked file, a write error and an unreadable source are reported instead of looking like "0 new". After the import, ↑, `:h /text` and the hints see these commands right away
- `:c` — clear the journal blocks
- `:json` / `:json <file>` — JSON viewer (the last block or a file)
- `:md <file.md>[#L<n>]` — Markdown handbook with formatting (click the name in the welcome message; Esc closes). It also accepts a path — absolute or relative to `md_dir`/cwd; that is how files from `:rg` and an Obsidian vault are opened; `#L<n>` opens right at line n (like on GitHub). `:md` is written to history (↑ / `:h`). Files longer than `md_render_lines` (1000 lines by default) are opened as source in the Line-API viewer — fast, with `/` search and `#L` jumps (formatted rendering of large files takes tens of seconds). `y` copies the full file path to the clipboard — in the formatted view you can also click the name in the header (in the raw view only `y`). A non-markdown file (docx, xlsx, pptx, doc/ppt/xls, rtf, epub, odt, pdf)? `:md` converts it: the converter comes from `md_converter` in `settings.yml`, otherwise the first one found (`anydoc` → `markitdown` → `pandoc`; recommended `pip install firecrawl-anydoc` — no dependencies, reads PDF locally, legacy `.doc`/`.xls` too). The markdown is cached in `mdcache/` in the data directory, so the viewer keeps `#L<n>`, `/` search and `y`. No converter — an install hint. **A scanned PDF is recognized locally**: with no text layer `:md` adds one via `ocrmypdf` (apt/dnf/apk/brew install ocrmypdf, which needs tesseract) — nothing leaves the machine; `md_ocr` in `settings.yml` disables it (`off`) or sets a custom command (`ocrmypdf -l rus+eng`), and with no engine the explicit "OCR is needed" message stays. Whether a file is text or a document is decided by content, not by extension
...
- `:term [path]` — a system terminal in a new window. Linux: `xdg-terminal-exec` / `gnome-terminal` / …; macOS: Terminal.app; Windows: `wt` or `cmd`. Custom: `$TERMINAL`. **A tab instead of a window** — `term_open: tab` in `settings.yml` (or `$IDVJPY_TERM_OPEN=tab`): `gnome-terminal --tab`, `konsole --new-tab`, `kgx`/`xfce4-terminal`/`mate-terminal`; a terminal without tabs (`alacritty`, `xterm`, `kitty` — the latter only via their server) gives an explicit error, not a window silently. The mode is shared by `:term`, `:new` and `& cmd`. **Switch it for this session:** `:term --tab` / `:term --window` (opens right away, does not touch `settings.yml`) — combinable with a path: `:term --tab /var/log`
...
- `:log [N]` (F7) — the full block output in a scrollable **Line-API** viewer (no 300-line truncation): rows scroll with ↑/↓ and pages with PgUp/PgDn, Esc/q; text search — `/` (Enter — forward, `n`/`N` — next/previous match, Esc — close the search field), the matching line is highlighted entirely (accent background + bold); `f` — keep only lines with matches (a repeated `f` or Esc restores the whole output, the line number in the subtitle is the original one), and then ↑/↓ walk the matches; Enter and Ctrl+C copy the highlighted line to the clipboard — as Enter in the F2 line cursor (with no search there is nothing selected, and the viewer says so instead of copying the first line; in the search field Ctrl+C copies the field text). `N` — blocks back (0 = the focused/last one). The lines are real (like F3), plus `STDERR`, if there was any. `y` copies the source file path (raw `:md` view); for block output — an explicit `No file path to copy`
...
- Highlighting of the focused block is soft: 12% of the theme's primary color over the block background (previously a solid `$primary-darken-1` was blinding, especially on a large `:?`; the brightness lift over the block background went 65 → 19 at 25% → ~9); it works in any theme
...
term_open: window            # where a terminal opens (`:term`, `:new`, `& cmd`): window — a new window, tab — a tab in an open one (gnome-terminal --tab, konsole --new-tab, …); $IDVJPY_TERM_OPEN=tab; :term --tab|--window for this session
...
Required (in `requirements.txt`): `textual==7.3.0`, `rich==14.3.0`, `pyperclip==1.11.0`, `PyYAML==6.0.3`, `Pygments==2.19.2`, `portalocker`.

Optional — external tools the app finds itself (or takes explicitly from `md_converter` / `md_ocr` / `editor`). They are not in `requirements.txt`: everything works without them, and `:md` names what is missing.

| What it gives | Tool | Install |
|---------------|------|---------|
| Documents in `:md`: docx/xlsx/pptx/doc/ppt/xls/rtf/epub/odt/pdf | `anydoc` — recommended | `pip install firecrawl-anydoc` (Rust, no dependencies, PDF read locally) |
| The same, if anydoc does not fit | `markitdown` | `pip install "markitdown[all]"` (pulls pandas/lxml/pdfplumber) |
| The same, a universal converter | `pandoc` | `apt/dnf/apk/brew install pandoc` |
| Scanned PDF: text layer locally | `ocrmypdf` + `tesseract` | `apt/dnf/apk/brew install ocrmypdf`; languages — `tesseract-ocr-<language>` |
| Markdown search in `:rg` | `ripgrep` (`rg`) | `apt/dnf/brew/pacman install ripgrep` (otherwise the built-in scanner) |
| External editor `:ed` | `nano` or `$EDITOR` / `$VISUAL` | `apt/dnf/apk/brew install nano` |
| File manager and terminal `:fm` / `:term` | `ranger` / `nnn` / `mc`, a terminal emulator | `$FILEMAN` / `$TERMINAL`, otherwise a list is searched |

Details on `:md` (converters, OCR, flags, cache, what every message means) — `:? md`.

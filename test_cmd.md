# План тестирования IDvjPy_term v1.1.5
...
### 2.1. Авто-раскрытие ссылок при сохранении (v1.1.5+)
```bash
# Сначала сохраним базовые команды
#start echo "Starting app"
#deploy systemctl restart nginx
#deploy nginx -t
#test python -m pytest

# Теперь сохраним команды со ссылками - они автоматически раскроются
#full-restart !deploy[1] && !start[1]
#check-nginx !deploy[2]

# Проверим, что ссылки раскрылись при сохранении
?full-restart
?check-nginx
```

**Ожидаемый результат**: Команды сохранены с раскрытыми ссылками:
- `full-restart[1]` содержит `systemctl restart nginx && echo "Starting app"`
- `check-nginx[1]` содержит `nginx -t`

### 2.2. Авто-раскрытие !! при сохранении
```bash
# Сохраним команду со сборкой через !!
#test-all !! start[1] test[1]

# Проверим результат
?test-all
```

**Ожидаемый результат**: Команда сохранена с раскрытой сборкой:
- `test-all[1]` содержит `echo "Starting app" python -m pytest`
...
- Авто-раскрытие ссылок `!`, `!!` работает при сохранении команд (v1.1.5+)

# План тестирования IDvjPy_term v1.172
...
### 12.1. Документы в `:md` (docx / pdf / xlsx / …)

Нужен конвертер: `pip install firecrawl-anydoc` (без него `:md` скажет, что поставить).
...
**Ожидание:** `:r` кладёт команду сфокусированного (или последнего) блока во ввод. `:cmd [N] [show]` — материализует команду блока с текущими `$VAR`/секретами и кладёт в буфер обмена; в журнале — маскированная версия, `show` печатает полную (секреты видны). `:cmd 5` при одном блоке — `too far back`. `cd` / `:cd` меняет cwd для shell-команд; нет каталога — ошибка, не молчание. **Строка целиком-путь работает как `cd` без слова `cd`** (`~/src`, `../lib`, `docs/`, `$PROJ` — тоже), `-` — как `cd -` (предыдущий каталог; без него явное `cd: OLDPWD not set`), а вот `./build.sh` по-прежнему **запускает** скрипт (путь-не-каталог остаётся командой), и имя из `$PATH` (`test`, `ls`) командой же и остаётся — даже если рядом лежит одноимённый каталог. База тегов / история / `.bashrc_term*` остаются в каталоге запуска (пустой `mytags.db` в новой папке не появляется). Серое приглашение слева в строке ввода (`~/путь ❯`) показывает текущий каталог и меняется после каждого `cd`/`:cd` (`~` вместо дома, длинный путь — хвостом; клик по пути возвращает фокус в строку). **Внутри git-репозитория рядом с путём — имя ветки** (`~/проект (main) ❯`; отделённый HEAD — `(@1a2b3c4)`, длинное имя режется по хвосту); читается из `.git/HEAD` без вызова `git` и обновляется после каждой команды (переключил ветку в приложении — подсказка сразу новая), `git_prompt: false` — выключить. Заголовок блока по-прежнему несёт полный путь. **На выходе каталог не теряется:** терминал получает OSC 7 (новая вкладка откроется в том же каталоге), а если cwd менялся, после выхода в stderr печатается готовая строка `cd '…'` — родительская оболочка за дочерним процессом сама не идёт (проверить: `cd /tmp`, `:q` — строка видна в терминале). Автоматически это делает обёртка: `IDVJPY_CWD_FILE=/tmp/f python3 app.py` → `cd "$(cat /tmp/f)"`. **Несколько окон (`:new`) файл не перехватывают:** `IDVJPY_CWD_FILE` не уезжает в дочерний запуск, поэтому файл пишет только то окно, чья обёртка его задала (иначе побеждал бы тот, кто закрылся последним). Проверить: с обёрткой выйти из окна `s2` — файл остаётся пустым/от главного окна. `:theme` показывает / ставит тему в `settings.yml`. `d` на журнале переключает dark/light.
...
5. `history_forget_not_found: false` в `settings.yml`, перезапуск, снова `Жр` — **Ожидание:** строка остаётся и в файле, и в ↑ (ошибка всё равно видна в журнале); вернуть `true` — снова убирается.

Автотест: `tests/test_history_typo.py` (4: helper `remove_history_file_line` с flock, опечатка уходит из файла и из ленты, `exit 127` без «command not found» остаётся, флаг `false` оставляет строку).
...
```
touch podfile.txt project.log 'my report.md'
cat po
kubectl get po
grep ot
...
# Главный сценарий: выделил мышью в выводе → сразу правый клик
протяжка мышью по блоку     # выделение скопировано в буфер при отпускании
<правый клик по блоку>      # выделенное вставлено в строку; буфер не перезаписан
<правый клик по пустой строке ввода с дрогнувшей мышью>  # буфер не тронут
```

**Ожидание:** правый клик в любом месте (журнал, список подсказок, пустая область) кладёт текст буфера в строку ввода — переводить туда курсор заранее не нужно, **фокус не прыгает** (Textual сам фокусировал виджет под мышью, из-за чего вставка как бы тормозила двумя перефокусами; теперь не-левая кнопка фокус не трогает). Вставка идёт в позицию курсора (а при выделении в строке — в конец, как Ctrl+V). Переносы строк из буфера становятся пробелами (поле однострочное): многострочная команда вставляется одной строкой целиком, ничего не теряется — ни через правый клик, ни через Paste-событие терминала (Textual сам брал бы только первую строку). Если строка длиннее поля, она целиком видна в сером превью под полем (с переносом по словам, до 6 строк, лишнее — `…`); короткая строка превью не показывает, а секретная — не показывает никогда (она и в поле замаскирована). **Выделенное мышью** копируется в буфер при отпускании кнопки, поэтому сценарий «выделил в выводе → правый клик» вставляет именно выделенное; сам правый клик выделения не заводит и не снимает (Textual заводил его на любую кнопку MouseDown, и дрогнувшая на клетку мышь отправляла в буфер случайный кусок под курсором — в приглашении это было `~ ❯`, что затирало данные человека). Подсветку после успешной вставки снимает сама строка ввода (фокус уходит в неё — `Input._watch_selection` в Textual чистит выделение экрана), но текст остаётся в буфере. Ссылки (`--seed`, `.md`, `:команды`, `?tag` в подсказках) правым кликом **не выполняются** — иначе он затирал бы только что вставленный текст (брокер `@click` в Textual не различает кнопки, отсечка — `CommandRunner._broker_event`). Правый клик по пункту списка подсказок ничего не выбирает (это левый клик). В построчном режиме (F2 / `:log`) правый клик всё равно вставляет буфер, а `Ctrl+V` там дописывает текущую строку. Пустой буфер — подзаголовок `Clipboard is empty`, строка не меняется. С `clear_clipboard_after_secret: true` вставка значения в `$$TOKEN=` очищает буфер и от правого клика. На модалках (JSON, markdown, просмотр вывода) вставки нет — они сами едят мышь.

Автотесты: `tests/test_paste_right_click.py` (12: вставка без фокуса в строке, в позицию курсора, выделенное вставляется и буфер не перезаписан, пустой буфер и выделение не тронуты, дрогнувший правый клик не кладёт `~ ❯` в буфер, ссылка правым кликом не выполняется, пункт подсказки не выбирается, фокус не прыгает на блок, блок не перехватывает фокус, очистка буфера после секрета, модалка), `tests/test_cwd_prompt.py` (превью длинной строки: короткая скрыта, длинная видна целиком, секретная не показывается, живой `$$`-секрет маскируется; переносы из буфера → пробел; Paste-событие терминала не теряет строки), `tests/test_mouse_selection.py` (протяжка левой кнопкой копирует в буфер — как раньше).
...
**Версия документа**: v1.118
**Версия приложения**: v1.172

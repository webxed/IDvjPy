...
**IDvjPy_term** v1.123 — умный терминал для создания командных строк из тегов.
...
- LLM из TUI: `:llm [провайдер] сообщение` (без имени — `default:`; `$OUT`/`$BLOCK` вставляют вывод блока; `@файл` вкладывает текст файла (UTF-8, ≤200 KB; можно несколько); контекст беседы — `history_turns: N` у провайдера (`:llm reset [<провайдер>|*]`); запросы пишутся в `history_*.txt`, но не в подсказки). Ответ — markdown и показывается форматированным (`llm_render_markdown`, см. настройки). Пока запрос идёт, в блоке крутится спиннер `⠋ thinking… 3s / 60s` (видно, что ждём ответ, и сколько допустимо)

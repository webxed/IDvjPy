...
**IDvjPy_term** v1.163 — умный терминал для создания командных строк из тегов.
...
- `:h import [оболочка]` — дописать историю оболочки пользователя в `history_<instance>.txt`: ищутся `~/.bash_history`, `~/.zsh_history`, fish (`~/.local/share/fish/fish_history`), ksh (`~/.sh_history`; `sh` — тот же файл), nushell (системный каталог данных: `Application Support` / `%APPDATA%`), PowerShell PSReadLine (`%APPDATA%\Microsoft\…` на Windows, XDG-путь на Linux/macOS); заданный `$HISTFILE` — первым (формат по содержимому, даже если имя файла нестандартное). Берутся последние 5000 строк каждого файла (у больших файлов — только хвост), уже имеющиеся строки не дублируются (повторный импорт ничего не добавляет), ничего не выполняется. Без имени — все найденные оболочки, `:h import zsh` — только она. Отказы видны явно: занятый файл, ошибка записи и нечитаемый источник сообщаются, а не выглядят как «0 новых». После импорта ↑, `:h /текст` и подсказки видят эти команды сразу

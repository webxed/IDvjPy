...
**IDvjPy_term** v1.150 — умный терминал для создания командных строк из тегов.
...
- `:export tag [file.json]` / `:import file.json` — один тег в JSON и обратно (импорт — всегда с новыми `tid`, схема — общая с CLI, `src/db_transfer.py`); `:export * [library.md]` — вся библиотека как Markdown-каталог
...
| Выложить свою библиотеку для команды | файл даёт CLI: `python3 backup_db.py export library.json` (в `backups/`), положить на https-хост и указать ссылку в `library_url` | `export library.json` |

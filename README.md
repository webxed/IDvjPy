...
**IDvjPy_term** v1.71 — умный терминал для создания командных строк из тегов.
...
- Внешний редактор: `:editor <файл>` (правки в файле), `:editor $OUT|$BLOCK` (вывод блока), `:editor` (пустой буфер). TUI на паузе (как `> cmd`). Редактор — `editor:` в `settings.yml` (можно с аргументами: `code --wait`), иначе `$VISUAL`/`$EDITOR`, иначе системный. В пути раскрываются `$VAR`/`$OUT` (`:editor $TMPDIR/pod-$OUT.json`); однострочный результат правит ввод (запуск — Enter), многострочный остаётся файлом с показанным путём (`@файл` / `| cmd`)

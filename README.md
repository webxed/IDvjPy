...
**IDvjPy_term** v1.85 — умный терминал для создания командных строк из тегов.
...
- **Хранение.** `secrets_<instance>.json` с правами `0600` — не в `.bashrc_term`, не в `history_*.txt` и не в playbook-логе; подхватывается при старте, по `:env` и `:session`.

...
**IDvjPy_term** v1.91 — умный терминал для создания командных строк из тегов.
...
| `:` | Команды приложения | `:q`, `:cd`, `:fm`, `:term`, `:session`, `:new`, `:welcome`, `:backup`, `:screensaver`, `:r`, `:cmd`, `:kctx`, `:playbook`, `:md`, `:?` |
...
- `:new [NAME]` (и `:session new [NAME]`) — запустить новое окно приложения в отдельном терминале: своя сессия (`.bashrc_term_<NAME>` / `history_<NAME>.txt`), общий data-каталог и БД тегов; секреты `$$` в новое окно не переносятся. Терминал — `$TERMINAL` (напр. `TERMINAL="kitty"` или `"alacritty -e"`), иначе из системных; команда запуска — `$IDVJPY_LAUNCH`

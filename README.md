...
**IDvjPy_term** v1.157 — умный терминал для создания командных строк из тегов.
...
- `:log [N]` (F7) — полный вывод блока в прокручиваемом **Line-API** просмотрщике (без обрезки в 300 строк): стрелки/PgUp/PgDn, Esc/q; поиск по тексту — `/` (Enter — вперёд, `n`/`N` — следующее/предыдущее совпадение, Esc — закрыть поле поиска), строка совпадения подсвечивается целиком (фон акцента + bold); `f` — оставить только строки с совпадениями (повторный `f` или Esc возвращают весь вывод, номер строки в подзаголовке — исходный). `N` — блоков назад (0 = сфокусированный/последний). Строки настоящие (как F3), плюс `STDERR`, если был. `y` копирует путь файла-источника (raw-вид `:md`); для вывода блока — явное `No file path to copy`

...
**IDvjPy_term** v1.179 — умный терминал для создания командных строк из тегов.
...
- MCP-сервер для AI-клиентов: `python3 mcp_server.py` — библиотека тегов по stdio (Claude Code, Cursor, …), только чтение (`:? mcp`)
...
- `> cmd` — настоящий TTY (htop, vim, ssh); клик и PgUp/PgDn активируют видимый блок журнала. Фоновая команда терминала не получает: `stdin` — `/dev/null`, поэтому `read`, `tsh`, `kubectl`, `ssh` не «крадут» клавиши и мышь (раньше `:kctx` с зависшим `tsh kube login` делал ввод нерабочим), а по таймауту/`F4` приложение возвращает терминал в свой режим. Нужен интерактив — `> cmd`, долгая задача без TTY — `@ cmd`; `:kctx <cluster>` запускает вход без `command_timeout`. **Прогресс-бар в журнале поэтому и не виден:** без TTY инструмент сам выключает прогресс — `git clone <url>` печатает только `Cloning into …` (и `remote:`-строки сервера, они идут по каналу данных), ровно как `git clone … | cat` в обычном терминале. Включить принудительно — `git clone --progress …`: каждая стадия (Enumerating/Counting/Receiving/…) останется отдельной строкой. Живого бара не будет всё равно: блок обновляется, когда команда завершилась (`[Executing...]` → вывод), а `\r`-перерисовки одной строки сворачиваются до итогового состояния (промежуточные проценты — сотни кадров — не сохраняются сознательно). Живьём прогресс видно через настоящий TTY: `> git clone …` (TUI на паузе; такой вывод в журнал не пишется).
...
# обёртка для shell (cwd следует за приложением): ./setup.sh --shell-helper
```
...
| `& cmd` | Выполнить в **отдельном терминале** (окно или вкладка — как `:term`; TUI продолжает работать; вывод — там, в журнале только факт запуска) | `& kubectl logs -f pod/api-1`, `& htop` |
...
| `:vault …` | Долговременное хранилище секретов с шифрованием по паролю (значения не показываются) | `:vault init`, `:vault cp SSH_PROD`, `:vault exec sshpass -- ssh u@h` |
...
### Хранилище секретов (`:vault`) — долговременное, с паролем

`$$VAR` живёт только сессию и исчезает при выходе. Для ключей, которые нужны всегда, — `:vault`: файл `vault.json.enc` в каталоге данных (0600), заголовок открыт, записи зашифрованы (scrypt + AES-256-GCM). Значение записи **никогда не печатается**. Нужен пакет `cryptography` (`pip install cryptography`); без него остальное приложение работает, а `:vault` сообщает об установке.

```text
:vault init                 # создать (пароль спрашивается дважды)
:vault unlock | lock        # открыть на сессию / забыть
:vault add SSH_PROD прод    # добавить/заменить значение (ввод маскирован)
:vault gen API_TOKEN 40     # сгенерировать значение
:vault list | rm NAME       # имена и подсказки / удалить
:vault cp SSH_PROD          # → буфер обмена (на экран не печатается)
:vault use SSH_PROD          # → $SSH_PROD в окружении этой сессии
:vault exec SSH_PROD=SSHPASS -- sshpass -p "$SSHPASS" ssh u@h   # значение в env, не в argv
:vault stdin PGPASS -- sudo -S true                              # значение на stdin
```

- **Инвариант.** Значение не попадает ни в журнал, ни в `:o`, ни в `↑`, ни в `history_*.txt`, ни в `:send`/`:export`; пока хранилище разблокировано, оно маскируется (`****`) как `$$`-секрет — даже если программа сама его напечатает. Пароль вводится только в маскированную модалку (в командной строке и истории его нет).
- **Куда уходит значение.** Только `cp` (буфер), `use`/`exec` (env дочернего процесса) и `stdin` (ввод команды) — в текст команды/argv оно не подставляется (`ps` его не видит). `cp` чистит буфер при выходе из TTY-сессии и через минуту — это самый слабое звено, буфер читают другие программы.
- **Сценарий с `> cmd`.** Скопировали `:vault cp SSH_PROD`, запустили `> ssh …`, вставили в приглашение; если программа спросит ещё что-то — это уже обычный ручной ввод в TTY. `exec` для программ, ждущих секрет в env (`sshpass`, `psql`, `mysql`, `vault`, `restic`, `borg`, `redis-cli`), переменную подбирает сам; для остальных пишите `NAME=VAR` явно.
- **Замок.** Пароль и значения живут только в памяти: `:vault lock` и выход снимают отданные в env переменные и забывают пароль; смена сессии (`:session`) снимает переменные сессии (сам файл хранилища общий и остаётся открытым). Подробнее — `:? vault`.
...
- `:cd [path]` — показать / сменить cwd для shell-команд (то же делает `cd path`). Строка, которая **целиком является путём**, работает как `cd` без слова `cd`: `~/src`, `../lib`, `/var/log`, `docs/`, а `-` — как `cd -` (в предыдущий каталог; если его нет — явное `cd: OLDPWD not set`). Работает и после подстановки — `$PROJ`. Правила без сюрпризов: имя из `$PATH` остаётся командой (`test`, `time`, `ls` запускаются, даже если рядом есть одноимённый каталог), а путь, который не каталог, уходит shell'у как раньше — `./build.sh` по-прежнему **запускает** скрипт, а не «входит» в него. При выходе терминал получает текущий каталог окна (OSC 7 — так же делает сама оболочка: новая вкладка/окно откроются там же), а если каталог всё-таки менялся, в stderr печатается готовая строка `cd '…'` — родительская оболочка за дочерним процессом сама не пойдёт. Автоматически это делает обёртка (как ranger/nnn):

```bash
idvjpy() {                     # каталог shell следует за приложением
  local f; f=$(mktemp)
  IDVJPY_CWD_FILE=$f command idvjpy "$@"   # pip-пакет; из клона — python3 ~/WibeCoding/Idivjopy/app.py
  cd "$(cat "$f")" 2>/dev/null
...
term_open: window            # где открывать терминал (`:term`, `:new`, `& cmd`): window — новое окно, tab — вкладка в открытом (gnome-terminal --tab, konsole --new-tab, …); $IDVJPY_TERM_OPEN=tab; :term --tab|--window — на сессию
...
`mcp_server.py` — сервер Model Context Protocol: AI-клиент (Claude Code, Cursor, …) спрашивает библиотеку напрямую — «есть ли у меня команда, чтобы посмотреть логи пода». Транспорт stdio: процесс запускает сам клиент, портов приложение не слушает. Только чтение: библиотека не меняется, команды не выполняются, секреты не читаются.

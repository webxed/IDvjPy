...
**IDvjPy_term** v1.146 — умный терминал для создания командных строк из тегов.
...
- `:h import [оболочка]` — дописать историю оболочки пользователя в `history_<instance>.txt`: ищутся `~/.bash_history`, `~/.zsh_history`, fish (`~/.local/share/fish/fish_history`), ksh (`~/.sh_history`), nushell и PowerShell PSReadLine (`%APPDATA%\Microsoft\PowerShell\PSReadLine\ConsoleHost_history.txt` на Windows, XDG-путь на Linux/macOS); заданный `$HISTFILE` — первым. Берутся последние 5000 строк каждого файла, уже имеющиеся строки не дублируются (повторный импорт ничего не добавляет), ничего не выполняется. Без имени — все найденные оболочки, `:h import zsh` — только она. После импорта ↑, `:h /текст` и подсказки видят эти команды сразу

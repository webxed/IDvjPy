...
**IDvjPy_term** v1.102 — умный терминал для создания командных строк из тегов.
...
- `:log [N]` (F7) — полный вывод блока в прокручиваемом **Line-API** просмотрщике (без обрезки в 300 строк): стрелки/PgUp/PgDn, Esc/q. `N` — блоков назад (0 = сфокусированный/последний). Строки настоящие (как F3), плюс `STDERR`, если был
...
| `F7` | Полный вывод блока в Line-API просмотрщике — то же, что `:log` |

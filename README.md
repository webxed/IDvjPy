...
**IDvjPy_term** v1.65 — умный терминал для создания командных строк из тегов.
...
- LLM из TUI: `:llm [провайдер] сообщение` (без имени — `default:`; `$OUT`/`$BLOCK` вставляют вывод блока; `@файл` вкладывает текст файла (UTF-8, ≤200 KB; можно несколько); запросы пишутся в `history_*.txt`, но не в подсказки)

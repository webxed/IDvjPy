...
- Runbook — полуавтоматическая цепочка команд: `:run <tag|файл.yml>` выполняет шаги подряд и останавливается там, где нужно ваше решение (`run:manual` — строка во вводе, правите и Enter; `run:prompt` — набираете с нуля; пустой Enter пропускает шаг). Ошибка шага останавливает прогон, `Esc` / `:run stop` — тоже; `--step` — остановка на каждом шаге, `--dry` — только план; справка — `:? run`. Готовый пример — `:run vapprole` (токен → роль → `role_id` → `secret_id` → login → проверка)
...
| `↑` / `↓` | Во вводе: лента сессии (всё набранное — `:`-команды, `?теги`, `!ссылки`, `#теги`, `$VAR=…`; кроме `$$секретов`) + отфильтрованный `history_<instance>.txt`; набранный текст фильтрует совпадения; прокрутка журнала, если фокус на блоке |

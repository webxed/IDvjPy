...
**IDvjPy_term** v1.114 — умный терминал для создания командных строк из тегов.
...
| `:` | Команды приложения | `:q`, `:cd`, `:fm`, `:term`, `:session`, `:new`, `:send`, `:welcome`, `:backup`, `:screensaver`, `:r`, `:cmd`, `:kctx`, `:playbook`, `:md`, `:rg`, `:?` |
...
- `:md <file.md>` — справочник Markdown с форматированием (клик по имени в приветствии; Esc закрывает). Принимает и путь — абсолютный или относительно `md_dir`/cwd, так открываются файлы из `:rg` и Obsidian-vault
- `:rg <паттерн> [каталог]` — поиск по markdown-файлам: ripgrep, если установлен (иначе встроенный сканер + подсказка по установке). Паттерн — регулярное выражение, «умный регистр» (нет заглавных — регистр не важен). База — `[каталог]`, иначе `md_dir` из `settings.yml`, иначе cwd. Результаты — фрагменты с кликабельными `путь:строка` (открываются встроенным md-просмотрщиком); `:rg <N>` открывает N-й результат (1-based). Пропускает скрытые и служебные каталоги (`.git`, `.obsidian`, `node_modules`, …)
...
- `:new [NAME|-] [DIR]` (и `:session new …`) — запустить новое окно приложения в отдельном терминале: своя сессия (`.bashrc_term_<NAME>` / `history_<NAME>.txt`), общий data-каталог и БД тегов. `DIR` — рабочий каталог новой сессии (по умолчанию data-каталог); имя `-`/пусто — авто `sN`. Секреты `$$` не переносятся. Кнопка `New session` в футере / `Ctrl+N`. Терминал — `$TERMINAL` (напр. `kitty` / `alacritty -e`), иначе из системных; запуск — `$IDVJPY_LAUNCH`
- `:send <сессия|*> <команда>` — переслать команду в другую сессию (окно `:new`): вставляется во ввод целевой сессии, запуск там — отдельным Enter. `:send!` — выполнить сразу (`:send! <сессия|*> <команда>`). `*` — всем сессиям, кроме своей. Команда материализуется у отправителя (`$VAR`/`$OUT`, алиасы, а также `|@label`/`|@N` → `<источник> | <команда>`), но значения секретов `$$` маскируются (`****`) и в ящик не пишутся. Метки буферов — свои в каждой сессии, поэтому `|@…` раскрывается в полный вызов (источник в целевой сессии выполнится заново); нет такой метки у отправителя — команда не отправляется. Обмен — через `inbox_<сессия>.jsonl` (0600) в data-каталоге; сообщение для незапущенной сессии ждёт её старта. Приходит и во время набранного текста — дописывается в конец, не затирая. Tab после `:send ` подсказывает имена сессий (`*` — всем остальным; текущая помечена)
...
- `:log [N]` (F7) — полный вывод блока в прокручиваемом **Line-API** просмотрщике (без обрезки в 300 строк): стрелки/PgUp/PgDn, Esc/q; поиск по тексту — `/` (Enter — вперёд, `n`/`N` — следующее/предыдущее совпадение, Esc — закрыть поле поиска), совпадение подсвечивается. `N` — блоков назад (0 = сфокусированный/последний). Строки настоящие (как F3), плюс `STDERR`, если был
...
| `F7` | Полный вывод блока в Line-API просмотрщике — то же, что `:log` (поиск внутри — `/`, `n`/`N`) |

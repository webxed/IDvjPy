...
**IDvjPy_term** v1.70 — умный терминал для создания командных строк из тегов.
...
- Внешний редактор: `:editor <файл>` (правки в файле), `:editor $OUT|$BLOCK` (вывод блока), `:editor` (пустой буфер). TUI на паузе (как `> cmd`). Редактор — `editor:` в `settings.yml` (можно с аргументами: `code --wait`), иначе `$VISUAL`/`$EDITOR`, иначе системный. Однострочный результат правит ввод (запуск — Enter), многострочный остаётся файлом с показанным путём (`@файл` / `| cmd`)

...
IDvjPy_term (v1.100) is a Python terminal application (TUI) built with the Textual framework. It provides a keyboard-driven interface for running shell commands with persistent, tagged command history stored in SQLite.

Philosophy: tags are variables holding command templates; the app assembles them into command lines (`!tag[tid]`, `!!`).

Bump `CommandRunner.VERSION` minor on every commit (`v1.100` → `v1.101`). `:update` compares that string with GitHub `main` (`https://github.com/webxed/IDvjPy`).
...
- **`src/version_bump.py`** (launcher `bump_version.py`): bump `CommandRunner.VERSION` minor and sync every release file in one run (`--set`, `--dry-run`, `--check`); `check()` mirrors `tests/test_release_meta.py`

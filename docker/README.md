...
| `$$TOKEN=sk-…` | секрет: значение скрыто в журнале, файл секретов удаляется при выходе |
| `$SECRET_ID=@secret_id` | значение из табличного вывода блока (vault `secret_id  …`) |
| `:cmd` | материализованная команда блока с текущими `$VAR`/секретами → буфер |
| `:o` / `:o /text` | последние выводы сессии / поиск по ним (переживают `:c`) |
| `:watch 2 date` | перезапуск команды каждые N секунд в одном блоке |

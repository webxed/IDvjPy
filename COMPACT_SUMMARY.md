...
TUI на Textual для запуска shell-команд с тегированной историей в SQLite. Версия: **v1.37**.
...
| `:` | `:q` `:w` `:h` `:c` `:json` `:i` `:?` `:cd` `:fm` `:term` `:env` `:session` `:welcome` `:backup` `:screensaver` `:r` `:/` `:g` `:n` `:N` `:export` `:import` `:theme` `:md` `:playbook` `:update` `:kill` `:watch` |
...
| File | Coverage |
|------|----------|
| `test_cmd.md` | Manual plan v1.6 (app v1.37) |
...
| `src/app.py` | TUI (`CommandRunner`), v1.37 |

...
TUI на Textual для запуска shell-команд с тегированной историей в SQLite. Версия: **v1.41**.
...
| `:` | `:q` `:w` `:h` `:c` `:json` `:i` `:?` `:cd` `:fm` `:term` `:env` `:session` `:welcome` `:backup` `:screensaver` `:r` `:/` `:g` `:n` `:N` `:export` `:import` `:theme` `:md` `:playbook` `:update` `:kill` `:watch` `:mv` `:stats` `:diff` |
...
| File | Coverage |
|------|----------|
| `test_cmd.md` | Manual plan v1.6 (app v1.41) |
...
| `src/app.py` | TUI (`CommandRunner`), v1.41 |

...
| `src/md_convert.py` | Документы (docx/xlsx/pptx/doc/ppt/xls/rtf/epub/odt/pdf) → markdown для `:md`: `is_plain_text` решает «текст или контейнер» **по содержимому** (сигнатуры PDF/OLE2/RTF, NUL-байты, части офисного ZIP, доля печатных байтов у не-UTF-8), `find_converter` — `md_converter` → `$IDVJPY_MD_CONVERT` → первый из `anydoc`/`markitdown`/`pandoc` (явный не подменяется). `anydoc` (`pip install firecrawl-anydoc`, Rust, без зависимостей, GIL отпущен) вызывается импортом без `ocr`; остальные — внешние процессы (`pandoc` → `--to gfm --wrap=none`), таймаут 30 с. **Скан-PDF:** если текста нет (`needs_ocr`/пусто) и файл — PDF, `_convert_with_ocr` зовёт `ocrmypdf` во временном каталоге (`--skip-text --optimize 0 --output-type pdf`, если режим не задан вручную) и повторяет конвертацию по текстовому слою; движок — `md_ocr` → `$IDVJPY_MD_OCR` → автопоиск (`off` выключает), таймаут 300 с (`OCR_TIMEOUT`), результат кэшируется отдельным ключом (`конвертер|команда OCR`), в сообщениях пара видна как `anydoc+ocrmypdf`, а явно заданный, но отсутствующий движок — `ocr_missing`. Кэш `<data>/mdcache/<имя>-<ключ>.md` (путь+mtime+размер+конвертер) читается **до** проверки доступности (переживает удаление конвертера), просмотрщик открывает **исходный** путь; в сеть не уходит ничего |
...
- **Скан-PDF распознаётся локально: `ocrmypdf` + tesseract.** Если конвертер текста не нашёл (`needs_ocr` от anydoc или пустой ответ), файл — PDF и есть движок, `:md` накладывает текстовый слой через `ocrmypdf` и читает PDF заново. Новый ключ `md_ocr` (settings.yml): пусто — автопоиск `ocrmypdf`; `off`/`none`/`no`/`false`/`0` — не распознавать (остаётся сообщение «нужен OCR»); `on`/`auto` — как пусто; своя команда — свои флаги (напр. `ocrmypdf -l rus`). Разово — `$IDVJPY_MD_OCR`. Свои флаги добавляются только к `ocrmypdf` и только если режим не задан вручную: `--skip-text` (без него он отказывается от PDF, где часть страниц с текстом), `--optimize 0 --output-type pdf` (нужен только текст, без сжатия и PDF/A) и `--sidecar`. Распознавание идёт в рабочем потоке, таймаут 300 с, результат кэшируется **отдельным ключом** (`конвертер|команда OCR`), в сообщениях пара видна как `anydoc+ocrmypdf`. Наружу ничего не уходит: `ocr="hosted"` у anydoc не используем.
- **Два отказа OCR, найденные живой проверкой.** (1) Без языкового пакета мы показывали хвост stderr ocrmypdf (подсказку про `chi_sim`) — теперь `_failure_detail` выбирает строку с причиной, а `_missing_language` превращает это в отдельную ошибку `ocr_language` с кодом языка («поставьте пакет `tesseract-ocr-<язык>` или задайте свой язык в `md_ocr`»). (2) `anydoc` отказывается читать «картиночный» PDF даже с OCR-слоем (её свой порог по размеру страницы: мелкие и нестандартные сканы) — раньше путь заканчивался чужой внутренней ошибкой; теперь ocrmypdf пишет ещё и текстовый `--sidecar`, и если конвертер отказался, `:md` отдаёт распознанный текст (конвертер показывается как `ocrmypdf`), так что OCR не заходит в тупик. Ловушка YAML закрыта: `md_ocr: off` без кавычек PyYAML читает булевым `False` — `CommandRunner._ocr_setting` понимает это как «выключено», а не как «авто».
- **Живые проверки, а не только заглушки.** Прогнано с настоящими `ocrmypdf` 17.12.1 и `tesseract` 5.3.4: сверил флаги с реальным CLI (`-s/--skip-text`, `-O/--optimize 0`, `--output-type pdf`, `-l`, `--sidecar`), английский скан → markdown за 1.2 с, повторный `:md` — из кэша; русский скан без пакета давал латиницу («Квартальный отчёт» → «KBapTaJIbHbIN OTYET»), с `apt install tesseract-ocr-rus` и `md_ocr: ocrmypdf -l rus` — корректный русский текст (`rus+eng` — тоже). Тесты: `tests/test_md_convert.py` (40; три живых — любой отсутствующий инструмент/пакет пропускает свой случай, остальное закрыто фейковым `ocrmypdf` в PATH).
- **Зависимости отражены в документации.** Новый подробный раздел справки `:? md` (75–78 строк в каждом языке): обе команды, решение «текст или документ» по содержимому, конвертеры с покрытием форматов и командой установки (anydoc — рекомендуем, markitdown, pandoc, своя команда), OCR целиком (установка по ОС, tesseract и языковые пакеты, все значения `md_ocr`, какие флаги и почему, `--sidecar`, таймаут), кэш, расшифровка каждого сообщения и список настроек. В трёх README раздел «Зависимости» переписан: обязательные (из `requirements.txt`) отдельно, необязательные внешние инструменты — таблицей с командами установки (anydoc/markitdown/pandoc, ocrmypdf + `tesseract-ocr-<язык>`, ripgrep, nano, проводник/терминал). Обновлены `CLAUDE.md`, `AGENTS.md` (OCR локальный, языковой пакет), `docker/README.md` (в образе OCR нет), комментарии в трёх шаблонах `settings/*.yml`, `COMPACT_SUMMARY.md`, `test_cmd.md`.

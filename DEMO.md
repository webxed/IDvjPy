...
Версия приложения: **v1.179**. Длительность живого рассказа: **12–15 минут**.
...
**Язык тура.** Подписи берутся из слоя языка: базовый `src/demos/<имя>.yml` хранит шаги и базовый (русский) текст, перевод — в `src/demos/text/<lang>/<имя>.yml` (`title`, `captions`, `types` по номеру шага; команды, `keys`, `pause` и `loop` общие). Тур говорит на языке интерфейса: `python3 app.py --lang ru --demo short` или `language: ru` в `settings.yml`. Английские подписи — `src/demos/text/en/`, добавить новый язык — создать `src/demos/text/<lang>/` по образцу (проверка — `python3 -m pytest tests/test_demo.py -q`).
...
- **C. Теги.** `#api`, `?chain[1]`, `??`, `!api[1]` + Enter, `!! api[1] && logs[1]`, `#api+1` (правка → второй Enter), `#api=2=…`, `#api-1` → `?api` → `#api!1`, `:mv tmp[1] logs`, `:stats`, `:export * library.md`, `:alias api run.sh`. Для мыши есть ускорение: в `??` обычный клик по ссылке только подставляет `!tag[tid]`, а **Ctrl+клик** (или двойной клик) — подставляет и сразу выполняет (то же, что набрать ссылку и дважды нажать Enter); нужен `terminal_mouse: true`.
- **D. Обвязка.** `$HOST=localhost` + `echo ping $HOST`, `:env`, `:h 8`, `:c` и `:o /CrashLoop` (память выводов переживает очистку), `:diff` двух `printf`, `:r 1`, `:watch 1 date +%s` / `:watch stop`, `@ sleep 60` + F4, `:backup`, `:kctx`, `:screensaver 120` / `:screensaver matrix` / `:screensaver stars` / `:screensaver 0`, `#`-комментарий в историю.
...
| Команда в своём терминале | `& sleep 30` | отдельное окно (или вкладка — `term_open`) терминала; TUI продолжает работать |

Фраза: «Обычный Enter — вывод в журнал. `>` — если нужен настоящий терминал. `&` — если команде нужно своё окно.»
...
| Справочник / поиск | `:md SEED_LINUX_COMMANDS.md` / `:rg <текст>` | форматированный Markdown в модалке; `:rg` — фрагменты markdown, клик по `путь:строка` открывает на строке |
| Документ как markdown | `:md report.docx` / `:md report.pdf` | не-markdown файл (docx/pdf/xlsx/rtf/…) конвертируется в markdown; нужен конвертер (`pip install firecrawl-anydoc` — без зависимостей, или markitdown/pandoc), результат кэшируется в `mdcache/` |
...
3. `>` только когда нужен TTY, `&` — когда команде нужно своё окно; всё остальное остаётся в журнале.

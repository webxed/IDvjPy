...
IDvjPy_term хранит тегированные команды в SQLite. Этот файл описывает текущую схему, кэш в памяти и все пути чтения (состояние на **v1.119**).

Код: `src/database_v2.py` (доступ к SQLite), `src/app.py` (маршрутизация `?`, `!`, `!!`, Tab, старт).

При первом запуске файл БД создаётся пустым. Если живых команд нет (`has_live_commands` = false), в журнале показывается каталог seed-скриптов: клик по `--seed` вставляет команду во ввод, клик по `.md` или `:md файл.md` открывает справочник.
...
Почти все выборки команд фильтруют `deleted = 0`. Удалённые через `#tag-` / `#tag-tid` / `#name--` в списках и выполнении не видны. `??` показывает полностью спрятанные теги (нет живых строк) и подсказку `#tag!` / `#name!!`.
...
- `get_hidden_tags()` — теги, у которых нет живых строк (только `deleted = 1`); внизу списка блок Hidden с `#tag!` / `#name!!`
...
- тот же блок Hidden, что у `?` (спрятанные теги **не** попадают в `!`-completion)
...
| `get_hidden_tags` | теги только с `deleted = 1` (нет живых строк) |

...
IDvjPy_term хранит тегированные команды в SQLite. Этот файл описывает текущую схему, кэш в памяти и все пути чтения (состояние на v1.1.18).

Код: `database_v2.py` (доступ к SQLite), `app.py` (маршрутизация `?`, `!`, `!!`, Tab, старт). Старый модуль `database.py` приложение не использует.
